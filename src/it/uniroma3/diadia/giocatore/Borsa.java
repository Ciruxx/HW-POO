...
//		attrezzi[k] = attrezzi[this.numeroAttrezzi-1];
    ...
    public List<Attrezzo> getContenutoOrdinatoPerPeso(){
        Set<String> chiavi = this.attrezzi.keySet();
        List<Attrezzo> listaOrdinata= new LinkedList<>();
        while(chiavi.iterator().hasNext()) {
            int pesoCorrente=this.attrezzi.get(chiavi.iterator().next()).getPeso();
            if(listaOrdinata.iterator().next().getPeso()>pesoCorrente){
